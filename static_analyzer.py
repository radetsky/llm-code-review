...
class StaticAnalyzer:
    ...
    def analyze_diff(self, diff_content: str) -> Any:
        ...
        for line_info in added_lines:
            ...
            warnings.extend([f"STATIC_ANALYSIS: {issue}" for issue in critical_from_static])
    ...
    def _check_unsafe_functions(self, line: str, file_path: str) -> List[str]:
        ...
        for func in unsafe_functions:
            if func in line:
                issues.append(f"{file_path}: Unsafe function '{func.split('(')[0]}' detected")
                break
    ...
    def _check_hardcoded_urls(self, line: str, file_path: str) -> List[str]:
        ...
        for pattern in url_patterns:
            if re.search(pattern, line):
                ...
                if not any(skip in line.lower() for skip in ['localhost', '127.0.0.1', '0.0.0.0']):
    ...
    def _suggest_improvements(self, line: str, file_path: str) -> List[str]:
        ...
        if re.search(r'(password|api_key|secret|token)\s*=\s*["\']', line, re.IGNORECASE):
            suggestions.append(f"{file_path}: Consider using environment variables for sensitive configuration")
        
        # Suggest parameterized queries
        if 'SELECT' in line and ('+' in line or '%' in line):
            suggestions.append(f"{file_path}: Consider using parameterized queries to prevent SQL injection")
        
        # Suggest input validation
        if 'input(' in line:
            suggestions.append(f"{file_path}: Add input validation when using user input")
